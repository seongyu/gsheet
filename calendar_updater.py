...
def get_sheet_all_data(sheet_id):
  service = config.init('sheet')
  try:
    t = sheet.get_all_delevery_sheet(service, sheet_id)
    r = _grouping(t)
...
def get_sheet_data(sheet_id, arr):
  service = config.init('sheet')
  try:
    t = sheet.get_delevery_sheet(service, sheet_id, arr)
    r = _grouping(t)
    print('succssfully gettering from sheet..')
  except Exception as err :
    print('failed..',err)
